...
export function isSchemaType(value: any): value is SchemaType {
	return value instanceof Schema || value instanceof MapSchema || value instanceof ArraySchema;
}
...
export function findSchemaRecursive(
	...
	if (!skipRoot) {
		if (schema instanceof Schema || schema instanceof MapSchema || schema instanceof ArraySchema) {
			result.push(schema);
		}
	}
	...
	if (schema instanceof Schema) {
		// @ts-ignore
		Object.keys(schema.constructor._definition.schema).filter(
...
export function findStateRecursive(state: any, result: any[] = [], skipRoot = false) {
	if (!skipRoot) {
...
export function pairClientServer(clientObject: any, serverObject: any, holderMap: Map<string, Schema>) {
	setTimeout(() => { // wait to see if any max exceed error
		if (clientObject instanceof MapSchema) {
			(serverObject as MapSchema).onAdd((item, key) => {
				waitFor(() => clientObject.has(key), {
					...
					timeoutMs: 5000,
				}).then(() => {// TODO: should implement onAdd for clientObject for O(1)
			...
			Object.keys(clientObject).filter(k => !reservedKeys.includes(k)).forEach(key => {
			// @ts-ignore
				pairClientServer(clientObject[key], serverObject[key], holderMap);
			});
		} else if (typeof clientObject === 'object') {
			Object.keys(clientObject).filter(k => !reservedKeys.includes(k))
				.forEach(key => {
					pairClientServer(clientObject[key], serverObject[key], holderMap);
				});
		}
...
export async function waitFor<Func extends() => any, Options extends WaitForOptions, Return extends Options extends {skipTimeoutThrow: true} ? Truthy<Awaited<ReturnType<Func>>> | undefined : Truthy<Awaited<ReturnType<Func>>>>(
	func: Func,
	options: Options & WaitForOptions,
): Promise<Return> {
	const error = new Error(`waitFor "${options.waitForWhat}" failed: timeout ${options.timeoutMs}ms!`);
		...
		const amogus = async () => {
			try {
				...
				if (e instanceof Error) {
					reject(new Error(`waitFor "${options.waitForWhat}" failed: ${e.message}`));
				} else {
					reject(e);
				}
			}
